...
@testset "Autodiff basic tests" begin
    ...
    @test stats.solution ≈
          [6.46511403059392, 2.232708645245993, 0.66739749115448, 0.595756422923144,
           5.93267567755629, 5.527234565979492, 1.01332200852064, 0.4006682289818261] rtol = 1.0e-6
    @test stats.multipliers ≈
          [-2.3596898286423866, -6.205501994097753, -0.9276198475360146,
           -0.8472008503830812, 0.0] rtol = 1.0e-6
end
...
@testset "Autodiff specification file tests" begin
    ...
    @test stats.solution ≈ [-0.0945558739311759, 0.03151862464668532, 0.5157593121810534,
                            -0.4527220628824326, 0.031518624649457136] rtol = 1.0e-4
...
# The Algencan callbacks are plain top-level functions that read the active
# solver from the module-global `_CURRENT_SOLVER` reference, because the
# closure form of `@cfunction` errors on aarch64 with "closures are not
# supported on this platform". Any solve exercises that the C pointers can be
# built; the risk the shared reference carries is state leaking between solves,
# so check that an intervening solve of a different problem does not perturb a
# repeated solve.
...
@testset "CUTEst unconstrained test" begin
    ...
    @test stats.solution ≈ [-11.594439894369563, 13.203630043985436, -0.40343948846314803,
                            0.23677877472091655] rtol = 1.0e-6
...
@testset "CUTEst double bounded constraints test" begin
    ...
    @test stats.solution ≈ [78.0, 36.7758129, 29.995256, 45.0, 33.0] rtol = 1.0e-6
    @test stats.multipliers ≈ [403.26887955845586, 0.0, -809.4250334347122] rtol = 1.0e-6
...
@testset "CUTEst larger problem test" begin
    ...
    @test stats.solution ≈ [1.10000000,
                            0.00000000,
                            -0.51719452,
                            1.06418307,
                            -0.02550540,
